...
class AuthDataSource {
  ...
  Future<AuthModel> adminLogin(String username, String password) async {
    try {
      ...
      if (!response.containsKey('access_token') || !response.containsKey('refresh_token')) {
        throw ServerFailure('Phản hồi đăng nhập không chứa access_token hoặc refresh_token');
      }
      final authModel = AuthModel.fromJson(response);
      await apiService.setToken(authModel.accessToken, refreshToken: authModel.refreshToken);
      return authModel;
